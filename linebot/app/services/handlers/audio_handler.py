...
from app.services.handlers.common import create_quick_reply, send_message, show_loading_animation
...
def wait_for_audio_content(message_id: str, max_retries: int = 30, retry_delay: float = 0.5):
    """等待音訊內容可用 (blocking，因為不使用要錢錢的 push API)"""
    for i in range(max_retries):
        try:
            message_content = messaging_api_blob.get_message_content(message_id, async_req=False)
            
            if message_content is not None and len(message_content) > 0:
                logger.info(f"Got audio content after {i+1} retries, size: {len(message_content)} bytes")
            ...
            logger.debug(f"Error getting audio content, retry {i+1}/{max_retries}: {str(e)}")
            import time
            time.sleep(retry_delay)
            continue
...
async def handle_audio_message(event):
    ...
    try:
        ...
        if not message_content:
            ...
            await send_message(
                event.reply_token,
                [TextMessage(text="無法取得音訊檔案，請稍後再試", quick_reply=create_quick_reply())],
            )
            return
        ...
        if not text:
            await send_message(
                event.reply_token,
                [TextMessage(text="抱歉，無法辨識音訊", quick_reply=create_quick_reply())],
            )
            return
        ...
        # 傳入 Dify API
        response_text = await inference(f"以下為語音轉文字內容，可能會有少量錯字：{text}", user_id)
        ...
        await send_message(
            event.reply_token,
            [TextMessage(text="處理音訊時發生錯誤，請稍後再試", quick_reply=create_quick_reply())],
        )
    finally:
        # 清理檔案
